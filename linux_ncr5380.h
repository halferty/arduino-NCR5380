...
//INITIATOR COMMAND register is set.
...
//Note : a set bit indicates an active signal, driven by us or another device.
#define SR_RST      0x80
#define SR_BSY      0x40
#define SR_REQ      0x20
#define SR_MSG      0x10
...
//Setting a bit in this register will cause an interrupt to be generated when BSY is false and SEL true and this bit is
//asserted on the bus.
...
//Used in DMA transfer mode, data is latched from the SCSI bus on the falling edge of REQ (ini) or ACK (tgt)
...
//Convert status register phase to something we can use to set phase in the target register so we can get phase
//mismatch interrupts on DMA transfers.
